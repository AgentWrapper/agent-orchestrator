...
export function findAffectedSessions(
  ...
  return sessions.filter((session) => {
    ...
    if (event.prNumber !== undefined && (session.pr?.number === event.prNumber || session.prs?.some((p) => p.number === event.prNumber && (!event.repository || `${p.owner}/${p.repo}`.toLowerCase() === `${event.repository.owner}/${event.repository.name}`.toLowerCase())))) return true;
