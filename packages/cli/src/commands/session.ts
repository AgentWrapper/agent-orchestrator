...
export function registerSession(program: Command): void {
  ...
  session
    ...
    .option("--repo <owner/repo>", "Resolve a bare PR number against this repository")
    .option("--assign-on-github", "Assign the PR to the authenticated GitHub user")
    .action(
      async (
        prRef: string,
        sessionName: string | undefined,
        opts: { assignOnGithub?: boolean; repo?: string },
      ) => {
        const config = loadConfig();
        ...
        try {
          ...
          if (result.previousPr) {
            console.log(chalk.dim(`  Previous: ${result.previousPr.url}`));
          }
          console.log(
            chalk.dim(
