...
- **Windows**: tmux is **not** required. The default runtime on Windows is `process` (ConPTY via `node-pty`), and PowerShell is the default shell. See [CROSS_PLATFORM.md](./CROSS_PLATFORM.md) for what's different on Windows when contributing.
...
Your plugin package must satisfy the contract in [`PLUGIN_SPEC.md`](./PLUGIN_SPEC.md) — export a `PluginModule` with a valid manifest and `create()` function. The package must be published to npm before your registry PR is merged so `ao plugin install` can fetch it.

---

## Code Conventions

See [DEVELOPMENT.md](./DEVELOPMENT.md) for the full reference. The short version:
...
- Are there tests?
- Does it follow the TypeScript and shell conventions in [DEVELOPMENT.md](./DEVELOPMENT.md)?
