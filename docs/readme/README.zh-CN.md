...
<img src="../assets/readme/dashboard.png" alt="Agent Orchestrator 仪表盘，展示并行编程智能体会话" width="100%" />
</div>
...
<table>
  <tr>
    ...
    <td width="64%">
      <img src="../assets/readme/dashboard.png" alt="Agent Orchestrator 看板，展示多个并行会话" />
    </td>
  </tr>
  <tr>
    <td width="36%">
    ...
    <td width="64%">
      <img src="../assets/readme/session-terminal.png" alt="Agent Orchestrator 内的会话终端" />
    </td>
  </tr>
  <tr>
    <td width="36%">
    ...
    <td width="64%">
      <img src="../assets/readme/reviews-tab.png" alt="显示评审运行与操作的 Reviews 标签页" />
    </td>
  </tr>
  <tr>
    <td width="36%">
    ...
    <td width="64%">
      <img src="../assets/readme/browser-preview.png" alt="显示本地应用预览的浏览器预览标签页" />
    </td>
  </tr>
</table>
...
| 文档                                                             | 在需要以下内容时从这里开始                                                                   |
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| [docs/architecture.md](../architecture.md)                     | 后端心智模型、生命周期、持久化、CDC、状态推导与 daemon 边界。                                |
| [docs/backend-code-structure.md](../backend-code-structure.md) | 包职责归属以及各后端关注点应放在何处。                                                       |
| [docs/cli/README.md](../cli/README.md)                         | CLI 行为与 daemon 路由映射。                                                                 |
| [docs/development.md](../development.md)                       | 本地开发的先决条件、构建步骤、运行测试与故障排除。                                           |
| [docs/STATUS.md](../STATUS.md)                                 | `main` 上当前已交付的内容，以及仍在进行中的部分。                                            |
| [docs/stack.md](../stack.md)                                   | 库、运行时与依赖决策。                                                                       |

## 遥测

Agent Orchestrator 的 Electron 渲染进程会向 PostHog 发送匿名使用事件，用于可靠性与产品理解。PostHog 会话录制默认关闭；若在限时调查中启用，本地路径与本地 URL 会在传输前脱敏。构建前将 `VITE_AO_POSTHOG_KEY` 设为空字符串可禁用传输。详见 [docs/telemetry.md](../telemetry.md)。
