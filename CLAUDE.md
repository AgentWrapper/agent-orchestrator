...
All app state, the daemon's data dir, `running.json`, worktrees, and the Tauri
supervisor's webview data dir (cache, cookies, local/session storage, crash
...
app-data location. `frontend/src-tauri/src/paths.rs` pins the webview data dir to
`~/.ao/tauri`; do not remove that override. See the hard rule in `AGENTS.md`.
