...
export default function CloudAppPage() {
  ...
  useEffect(() => {
    if (!api || visibleSessions.length === 0) {
      setSessionSCM({});
      return;
    }
    let cancelled = false;
    void Promise.all(
      visibleSessions.map(async (cloudSession) => {
        try {
          const result = await api.sessionSCM(cloudSession.id);
    ...
    // visibleSessionSCMKey keeps this independent from array identity churn on refresh.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [api, visibleSessionSCMKey]);
...
function cloudStatusView(status: CloudSession["status"]) {
  switch (status) {
    case "needs_input":
      return { label: "Needs input", className: "text-[#e8c14a]", cardClassName: "border-[#e8c14a]/20" };
    case "ci_failed":
      return { label: "CI failed", className: "text-[#ef6b6b]", cardClassName: "border-[#ef6b6b]/20" };
    case "changes_requested":
      return { label: "Changes requested", className: "text-[#e8c14a]", cardClassName: "border-[#e8c14a]/20" };
    case "pr_open":
      return { label: "PR open", className: "text-[#5b8def]", cardClassName: "border-[#5b8def]/20" };
    case "review_pending":
      return { label: "Review pending", className: "text-[#5b8def]", cardClassName: "border-[#5b8def]/20" };
    case "approved":
      return { label: "Approved", className: "text-[#74b98a]", cardClassName: "border-[#74b98a]/20" };
    case "mergeable":
      return { label: "Ready to merge", className: "text-[#74b98a]", cardClassName: "border-[#74b98a]/20" };
    case "merged":
      return { label: "Merged", className: "text-[#74b98a]", cardClassName: "border-[#74b98a]/20" };
    case "working":
      return { label: "Working", className: "text-[#f59f4c]", cardClassName: "border-[#f59f4c]/20" };
    case "terminated":
    case "exited":
      return { label: "Exited", className: "text-[#646a73]", cardClassName: "border-white/[0.06]" };
    case "idle":
    default:
      return { label: "Idle", className: "text-[#9ba1aa]", cardClassName: "border-white/[0.06]" };
  }
}
