...
<Callout type="info">
	**PR metadata is captured automatically.** Non-Claude-Code agents (Codex, Codex Fugu, Cursor, Aider, OpenCode) use
	...
	PR number and branch so the dashboard stays in sync. Claude Code uses PostToolUse hooks in `.claude/settings.json`
	instead — the same metadata is written through Claude Code's native hook system. Both approaches are set up
	transparently by AO; you don't need to configure anything.
</Callout>
