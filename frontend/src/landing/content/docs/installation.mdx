...
The recommended way to install AO is the desktop build from GitHub Releases. Download the latest build for your platform:
...
<Callout type="warn" title="npm is no longer recommended">
	The `@aoagents/ao` npm package is frozen and no longer receives updates. Prefer the desktop build above. The package
	still works if you want the `ao` CLI:
</Callout>
