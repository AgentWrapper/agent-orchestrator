...
Built-in reviewer harnesses are `agy`, `claude-code`, `codex`, `continue`, `copilot`, `cursor`, `goose`, `kilocode`, `kiro`, `opencode`, `pi`, `qwen`, and `vibe`. Kilo Code requires the
...
Agy, Continue, Goose, Qwen, and Vibe reviewer support is experimental and host-trusted. Their sandbox, readonly, plan, or
