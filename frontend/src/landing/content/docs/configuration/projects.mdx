...
Identity fields such as `projectId`, `path`, `storageKey`, `originUrl`, and `projectPrefix` belong to the global registry, not the local config.
...
Set `projectPrefix` in the global project registry or through project registration/settings. Project prefixes must use letters, numbers, underscores, or hyphens. Existing `sessionPrefix` config is still accepted as a compatibility alias.
