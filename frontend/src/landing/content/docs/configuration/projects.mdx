...
Built-in reviewer harnesses are `aider`, `agy`, `amp`, `claude-code`, `codex`, `continue`, `copilot`, `cursor`, `devin`, `droid`, `goose`, `kilocode`, `kiro`, `kimi`, `opencode`, `pi`, `qwen`, and `vibe`. Kilo Code requires the
...
Agy, Continue, Devin, Droid, Goose, Kimi, Qwen, and Vibe reviewer support is experimental and host-trusted. Their readonly, plan, or
