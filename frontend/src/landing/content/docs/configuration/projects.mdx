...
Claude Code worker sessions default to no MCP servers, even if your user-level Claude config has MCP servers enabled for the orchestrator. Opt workers into a specific set under the worker role:
...
Use `mode: inherit` when a worker should intentionally load Claude Code's normal MCP configuration, or `mode: none` to make that isolation explicit.
