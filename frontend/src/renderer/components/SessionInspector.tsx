...
function updateSessionAutoInjectPolicy(
	...
	return workspaces?.map((workspace) => ({
		...workspace,
		sessions: workspace.sessions.map((candidate) =>
			candidate.id === sessionId ? { ...candidate, autoInjectReviewFeedback } : candidate,
		),
	}));
}
...
function ReviewsSection({
	...
	const autoInjectPolicy = useMutation({
		...
		onMutate: async (autoInjectReviewFeedback) => {
			...
			queryClient.setQueryData<WorkspaceSummary[]>(
				workspaceQueryKey,
				updateSessionAutoInjectPolicy(previous, session.id, autoInjectReviewFeedback),
			);
			return { previous };
		},
		onError: (_err, _value, ctx) => {
			queryClient.setQueryData(workspaceQueryKey, ctx?.previous);
		},
		onSettled: () => {
			void queryClient.invalidateQueries({ queryKey: workspaceQueryKey });
		},
	...
	const autoInjectPolicyError =
		autoInjectPolicy.error instanceof Error ? autoInjectPolicy.error.message : null;
