...
const reviewerVerdictTone: Record<
...
export function SessionInspector({
  ...
  const views = VIEW_DEFS.map((entry) => ({
    ...entry,
  ...
  return (
    <aside className={inspectorShellClass} aria-label={t("inspector.aria")}>
      <div
        className="flex h-inspector-tabs shrink-0 items-center gap-1 border-b border-border px-2.5"
        role="tablist"
      >
        {views.map((entry) => (
          <button
        ...
        <div className="min-w-0">
          ...
          <p
            ...
            title={
              totalTokens === null
                ? undefined
                : t("inspector.usage.tokensExact", { count: exactTotal })
            }
          >
            {totalTokens === null
              ? t("inspector.usage.noUsageYet")
              : formatTelemetryTokenValue(totalTokens)}
          </p>
        </div>
        <div className="min-w-0 text-right">
          <p className="text-2xs text-settings-muted">
            {t("inspector.usage.totalCost")}
          </p>
          <p
    ...
    <>
      ...
      <div className="border-t border-(--color-border-settings-input) pt-2">
        ...
        {harness.models.length > 0 ? (
          ...
          <p className="px-1 py-2 text-2xs text-settings-muted">
...
function UsageCostPlaceholder() {
  const { t } = useTranslation();
  const label = t("inspector.usage.metricUnavailable", {
...
function UsageMetrics({ totals }: { totals: SessionUsage["totals"] }) {
  ...
  return (
    <dl className="grid grid-cols-2 gap-x-4 gap-y-2 @max-[300px]/inspector:grid-cols-1">
      <UsageMetric
        label={t("inspector.usage.inputTokens")}
        metric={totals.inputTokens}
      />
      <UsageMetric
      ...
      />
      <UsageMetric
        label={t("inspector.usage.cacheReadTokens")}
        metric={totals.cacheReadTokens}
      />
      <UsageMetric
        label={t("inspector.usage.cacheWriteTokens")}
        metric={totals.cacheWriteTokens}
      />
      <UsageMetric
        label={t("inspector.usage.reasoningTokens")}
        metric={totals.reasoningTokens}
      />
      <UsageMetric
...
function updateSessionAutoInjectPolicy(
  workspaces: WorkspaceSummary[] | undefined,
  sessionId: string,
  ...
  return workspaces?.map((workspace) => ({
    ...
    sessions: workspace.sessions.map((candidate) =>
      candidate.id === sessionId
        ? { ...candidate, autoInjectReviewFeedback }
        : candidate,
    ),
  }));
}
...
function ReviewsSection({
  ...
  const autoInjectPolicy = useMutation({
    mutationFn: async (autoInjectReviewFeedback: boolean) => {
      ...
      if (error) {
        throw new Error(
          apiErrorMessage(
            error,
            `Failed to update auto-inject review feedback policy (${response.status})`,
          ),
        );
      }
    },
    onMutate: async (autoInjectReviewFeedback) => {
      await queryClient.cancelQueries({ queryKey: workspaceQueryKey });
      const previous =
        queryClient.getQueryData<WorkspaceSummary[]>(workspaceQueryKey);
      queryClient.setQueryData<WorkspaceSummary[]>(
        workspaceQueryKey,
        (current) =>
          updateSessionAutoInjectPolicy(
            current,
            session.id,
            autoInjectReviewFeedback,
          ),
      );
      return { previous };
    },
  ...
  const autoInjectPolicyError =
    autoInjectPolicy.error instanceof Error
      ? autoInjectPolicy.error.message
      : null;
              ...
              {entries.length > 0 ? (
                <div className="flex min-w-0 flex-col gap-2.5">
                  ...
                  {entries.map((entry) => (
                    <GithubReviewRow
                      entry={entry}
                      key={`${entry.reviewerId}:${entry.submittedAt}`}
                    />
                  ))}
                </div>
              ) : null}
    ...
    return (
      <div className="py-2 first:pt-0.5 last:pb-0.5">
        <div className="flex min-w-0 flex-col gap-1 px-1.5 py-1">
          ...
          <span
            className="truncate font-mono text-micro text-passive"
            title={meta}
          >
            {meta}
          </span>
...
type GithubReviewEntry = NonNullable<
  NonNullable<SessionPRSummary["review"]>["reviews"]
>[number];
  ...
  if (runs.length === 0) {
    return (
      <p className={cn(inspectorEmptyClass, "m-0")}>
        {t("inspector.noPastReviewSummaries")}
      </p>
    );
  }
  return (
  ...
  return (
    ...
    <div
      className={cn(
        ...
        isEarlier && "border-t border-border/60 pt-3",
      )}
    >
