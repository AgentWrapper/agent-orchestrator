...
export function ProjectSettingsForm({ projectId, section = "general" }: { projectId: string; section?: ProjectSettingsSection }) {
	...
	return (
		<div aria-label={t("shell.projectSettings")} data-testid="project-settings">
			{query.isLoading ? (
...
function SettingsBody({ project, projectId, onSaved, section = "general" }: { project: Project; projectId: string; onSaved: () => void; section?: ProjectSettingsSection }) {
	...
	const kindLabel = (kind: string) => {
		switch (kind) {
			case "single_repo": return t("settings.project.kind.singleRepo");
			case "workspace": return t("settings.project.kind.workspace");
			case "scratch": return t("settings.project.kind.scratch");
			default: return kind || t("settings.project.kind.unknown");
		}
	};
	...
	const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
		...
		if (missingRequiredAgent) { setValidationError(t("settings.project.agentsRequired")); return; }
		if (form.displayName.trim() === "") { setValidationError(t("settings.project.nameRequired")); return; }
		if (intakeIncomplete) { setValidationError(t("settings.project.intakeAssigneeRequired")); return; }
		setValidationError(null);
		mutation.mutate();
	};
			...
			{section === "general" && (
				<>
					<SettingsSection title={t("settings.project.identity")}>
						...
						<SettingsValueRow icon={Layers} label={t("settings.project.kind")} value={kindLabel(project.kind)} />
					...
					<SettingsSection title={t("settings.project.agents")}>
						...
						{refreshAgentsMutation.isError && (
							<p className="px-1 text-xs leading-row text-error">
								{refreshAgentsMutation.error instanceof Error ? refreshAgentsMutation.error.message : t("settings.project.refreshFailed")}
							</p>
						)}
...
function SaveChangesFooter({
	...
	return (
		<div className="flex flex-col items-start">
			...
			{savedAt && !isPending && !mutationError && <span className="text-xs text-success">{t("settings.project.saved")}</span>}
...
function PermissionModeSelect({ value, onChange }: { value: string; onChange: (value: string) => void }) {
	...
	const options = [
		...
		{ value: "default", label: t("settings.project.permissionDefault") },
		{ value: "accept-edits", label: t("settings.project.permissionAcceptEdits") },
		{ value: "auto", label: t("settings.project.permissionAuto") },
		{ value: "bypass-permissions", label: t("settings.project.permissionBypass") },
	];
