...
export function CreateProjectFlow({
	...
	const openFolderStep = (kind: ProjectKind) => {
		if (!aoBridge.app.canChooseDirectory) {
			setError(null);
	...
	const submitManualPath = async (path: string) => {
		...
		setError(null);
		setValidationScan(null);
		if (selectedKind === "single_repo") {
			const setupCode = await repositorySetupRequired(trimmedPath);
			...
			{hasModePicker && (
				<>
					...
					<CreateProjectFolderDialog
						...
						onManualPathSubmit={() => void submitManualPath(manualPath)}
						onOpenChange={(open) => {
							if (!isBusy) {
...
function CreateProjectFolderDialog({
	...
	const isWorkspace = kind === "workspace";
	const canChooseDirectory = aoBridge.app.canChooseDirectory;
				...
				<Dialog.Content className="fixed left-1/2 top-1/2 z-50 flex max-h-[min(640px,calc(100svh-24px))] w-[min(640px,calc(100vw-24px))] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-lg border border-border bg-popover p-0 text-popover-foreground shadow-xl data-[state=open]:animate-modal-in">
					...
					<div className="min-h-0 overflow-y-auto px-4 py-4 sm:px-6 sm:py-6">
						{hasScan ? (
							...
							<form
								className="space-y-4 rounded-lg border border-border bg-background px-4 py-4"
								onSubmit={(event) => {
							...
							>
								<div className="space-y-1.5">
									<label className="text-[12px] font-medium text-muted-foreground" htmlFor="createProjectManualPath">
										Project path
									</label>
									<input
										autoFocus
										id="createProjectManualPath"
