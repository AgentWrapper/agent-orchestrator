...
export function TaskComposer({
  ...
  const isDirty =
    prompt.trim() !== "" || modelTouched || attachments.length > 0;
  useEffect(() => {
  ...
  const submitTask = async (interfaceMode?: "tui") => {
    ...
    try {
      const sessionId = await createTask({
        ...
        agent:
          agentTouched && agent
            ? (agent as CreateTaskInput["agent"])
            : undefined,
  ...
  return (
    <form onSubmit={submit} className="space-y-4 p-(--size-modal-padding)">
      <div className="space-y-1.5">
        ...
        />
        {attachmentError && (
          <p className="text-caption text-destructive">{attachmentError}</p>
        )}
        ...
        <div className="space-y-1.5">
          ...
          >
            {refreshAgentsMutation.isPending
              ? t("newTask.refreshingAgents")
              : t("newTask.refreshAgents")}
          </button>
        </div>
        <div className="space-y-1.5">
          <Label
