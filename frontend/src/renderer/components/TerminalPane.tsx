...
function bannerText(
  ...
  if (state === "error")
    return `Terminal error: ${error ?? "connection failed"}`;
  return undefined;
}
...
function AttachedTerminal({
  ...
  const pendingPreviewRef = useRef<{ sessionId: string; url: string } | null>(
    null,
  );
  ...
  const openLinkInBrowser = useCallback(
    (uri: string) => {
      ...
      // Serialize preview writes and coalesce queued output to its latest
      // target. A bounded request and unmount cleanup prevent a stalled or
      // stale pane from blocking later links forever.
      ...
      void (async () => {
        try {
          while (pendingPreviewRef.current) {
            ...
            let succeeded = false;
            try {
              const { error: previewError } = await apiClient.POST(
              ...
              if (previewError) {
                console.warn(
                  "Unable to open terminal link in Browser preview",
                  previewError,
                );
                continue;
              }
  ...
  const { attach, state, error, replaySettled } = useTerminalSession(
    attachSession,
    {
      daemonReady,
  ...
  if (initFailed) {
    return (
      <div className="grid h-full place-items-center bg-terminal p-4 font-mono text-xs text-muted-foreground">
        Terminal failed to initialize on this GPU/driver. Restart the app to
        retry.
      </div>
    );
  }
...
function ReplayCover() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(
      () => setShowLabel(true),
      REPLAY_COVER_LABEL_MS,
    );
    return () => window.clearTimeout(timer);
  }, []);
  return (
    <div
      className="absolute inset-0 grid place-items-center bg-terminal"
      data-testid="terminal-replay-cover"
    >
      {showLabel && (
        <div className="font-mono text-caption text-terminal-dim">
