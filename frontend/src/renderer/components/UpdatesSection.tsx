...
const CHANNEL_OPTIONS: UpdateChannel[] = ["latest", "nightly"];
			...
			<CardContent className="flex flex-col gap-4">
						...
						<SelectContent>
							{CHANNEL_OPTIONS.map((channel) => (
								<SelectItem key={channel} value={channel}>
									{t(channel === "latest" ? "settings.updates.channels.latest" : "settings.updates.channels.nightly")}
								</SelectItem>
							))}
						</SelectContent>
					</Select>
				...
				<div className="flex items-center gap-3">
					<Button type="button" variant="primary" onClick={() => save.mutate(form)} disabled={save.isPending}>
						{save.isPending ? t("settings.updates.saving") : t("settings.updates.save")}
					</Button>
					{save.isError && (
						<span className="text-xs text-error">
							{save.error instanceof Error && save.error.message
								? save.error.message
								: t("settings.updates.saveFailed")}
						</span>
					)}
			...
			<div className="flex items-center gap-3">
				...
				{status.state === "available" && (
					<Button type="button" variant="primary" onClick={() => void aoBridge.updates.download()}>
						{t("settings.updates.updateTo", {
							version: status.version ? `v${status.version}` : t("settings.updates.latest"),
						})}
					</Button>
				)}
...
function UpdateStatusLine({ status }: { status: UpdateStatus }) {
	...
	switch (status.state) {
		case "checking":
			return <span className="text-xs text-muted-foreground">{t("settings.updates.status.checking")}</span>;
		case "available":
			return (
				<span className="text-xs text-muted-foreground">
					{status.version
						? t("settings.updates.status.availableWithVersion", { version: status.version })
		...
		case "downloading":
			return (
				<span className="text-xs text-muted-foreground">
					{t("settings.updates.status.downloading", { percent: status.percent ?? 0 })}
				</span>
			);
		case "downloaded":
			return <span className="text-xs text-success">{t("settings.updates.status.downloaded")}</span>;
		case "not-available":
			return <span className="text-xs text-muted-foreground">{t("settings.updates.status.notAvailable")}</span>;
		case "unsupported":
			return <span className="text-xs text-passive">{t("settings.updates.status.unsupported")}</span>;
		case "error":
			return <span className="text-xs text-error">{status.message ?? t("settings.updates.status.failed")}</span>;
		default:
			return null;
	}
}
