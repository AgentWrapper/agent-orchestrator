	...
	return (
		<div className="flex h-full min-h-0 flex-col bg-background text-foreground" data-testid="board">
			...
			{!showWelcome && boardActionsInPanel && (boardLabel || actions) ? (
				<div
					className="center-panel-titlebar flex h-toolbar shrink-0 items-center gap-2 border-b border-border-strong"
					style={dragStyle}
				>
			...
			{archived.length > 0 && (
		<div className="relative shrink-0 border-t border-border-strong">
				{archiveExpanded && (
					<div
						...
						onPointerDown={(e) => { e.stopPropagation(); startArchiveResize(e); }}
				...
				{/* Full-width toggle row — the entire strip is the button. */}
				<button
					aria-expanded={archiveExpanded}
					aria-label={`Archive, ${archived.length} ${archived.length === 1 ? "session" : "sessions"}`}
					className={cn(
						"group flex w-full items-center gap-2 px-3 text-muted-foreground transition-colors hover:text-foreground",
				...
				>
					...
					<span className="font-sans text-2xs font-medium tracking-tight">Archive</span>
					<span className="ml-1.5 font-sans text-micro text-passive">{archived.length}</span>
					{archiveExpanded && (
						<div
			...
			<motion.div
				animate={{ height: archiveExpanded ? resolvedArchiveHeight : 0 }}
				initial={false}
			...
			>
				<div
					...
					className={cn(
						"board-scrollbar gap-2.5 overflow-y-auto px-3 pb-3",
						archiveLayout === "grid"
...
function LaneHeadingBar({ color }: { color: string }) {
	return <span aria-hidden="true" className="size-2.5 shrink-0 rounded-[2px]" style={{ background: color }} />;
}

function LaneHeadingText({ className, children }: { className?: string; children: string }) {
	return (
		<span className={cn("truncate font-sans text-xs font-semibold tracking-tight", className)}>
			{children}
		</span>
	);
}
...
const laneHeadingBandClass = "flex h-12 shrink-0 items-center gap-2.5 bg-surface-faint px-4";
...
function SplitLaneColumn({
	...
	// Only the idle/working column (zone="working") supports collapsing the idle sub-lane.
	const canCollapsePrimary = zone === "working" && showPrimary && showSecondary;
		...
		>
			{canCollapsePrimary ? (
				<button
					...
					className={cn(laneHeadingBandClass, "w-full cursor-pointer hover:bg-muted/30")}
				...
				>
					...
					<svg
						aria-hidden="true"
						className={cn("ml-1 size-icon-2xs shrink-0 text-muted-foreground transition-transform duration-normal", primaryCollapsed && "-rotate-90")}
			...
			{/* One scroller for the whole column. */}
			<div className="board-scrollbar min-h-0 flex-1 overflow-y-auto pb-3">
				{showPrimary ? (
					<motion.div
						animate={{ height: primaryCollapsed ? 0 : "auto" }}
						initial={false}
						transition={{ duration: 0.14, ease: [0.25, 0.46, 0.45, 0.94] }}
						style={{ overflow: "clip" }}
					>
						<div aria-label={primaryTone.regionLabel} className="px-3 pb-3 pt-4" role="region">
				...
				{showSecondary ? (
					<SecondaryLaneSection
						...
						topGap={showPrimary && !primaryCollapsed}
						tone={secondaryTone}
						onOpen={onOpen}
						onTerminate={onTerminate}
					/>
...
function SecondaryLaneSection({
	...
	return (
		...
		{!standalone && (
			<div className={cn("flex h-12 shrink-0 items-center gap-2.5 border-b border-border bg-surface-faint px-4", topGap ? "mt-3 border-t" : "")}>
