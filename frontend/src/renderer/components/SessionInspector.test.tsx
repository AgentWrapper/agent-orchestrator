...
import {
  sessionUsageDetailQueryKey,
  type SessionUsage,
} from "../hooks/useSessionUsage";
...
describe("SessionInspector Usage & cost section", () => {
  ...
  it("stays hidden and does not fetch detailed usage outside Developer Mode", async () => {
    ...
    expect(
      getMock.mock.calls.some(
        ([path]) => path === "/api/v1/usage/sessions/{sessionId}",
      ),
    ).toBe(false);
  });
  ...
  it("keeps API failures visible instead of silently hiding the section", async () => {
    ...
    const usageTitle = await screen.findByText("Usage & cost", undefined, {
      timeout: 2_500,
    });
    const usageSection = usageTitle.closest(
      "[data-testid='inspector-section']",
    ) as HTMLElement;
    expect(within(usageSection).getByRole("alert")).toHaveTextContent(
  ...
  it("hides the section when no usage event or token value exists", async () => {
    ...
    await waitFor(() =>
      expect(
        queryClient.getQueryData(sessionUsageDetailQueryKey("sess-1")),
      ).toEqual(expect.objectContaining({ harnesses: [] })),
    );
    expect(screen.queryByText("Usage & cost")).not.toBeInTheDocument();
  });
  ...
  it("shows token telemetry and expands agent and model details on click", async () => {
    ...
    expect(within(usageSection).getByText("Coming soon")).toHaveClass(
      "text-settings-muted",
    );
    expect(within(usageSection).getAllByText("1.2K").length).toBeGreaterThan(0);
    expect(
      within(usageSection).queryByText("1.2K tok"),
    ).not.toBeInTheDocument();
    ...
    expect(
      within(usageSection).getByText("Cache read tokens"),
    ).toBeInTheDocument();
    expect(
      within(usageSection).getByText("Cache write tokens"),
    ).toBeInTheDocument();
    expect(
      within(usageSection).getByText("Reasoning tokens"),
    ).toBeInTheDocument();
    expect(
      within(usageSection).getByText("Uncached input tokens"),
    ).toBeInTheDocument();
    expect(
      within(usageSection).getByTestId("session-usage-metrics"),
    ).toHaveClass("rounded-lg", "border", "bg-(--color-bg-settings-input)");
    ...
    expect(
      within(usageSection).queryByText("Unknown provider"),
    ).not.toBeInTheDocument();
    expect(
      within(usageSection).queryByText(/coverage|collecting/i),
    ).not.toBeInTheDocument();
    expect(
      within(usageSection).getByLabelText("Input tokens: 1,000 tokens"),
    ).toBeInTheDocument();
    ...
    expect(
      within(providerTrigger).getByLabelText("Cost telemetry unavailable"),
    ).toHaveTextContent("—");

    await user.click(providerTrigger);
    expect(providerTrigger).toHaveAttribute("aria-expanded", "true");
    const providerPeek = await screen.findByRole("region", {
    ...
    expect(
      within(providerPeek).queryByText("Coming soon"),
    ).not.toBeInTheDocument();
    ...
    expect(
      within(modelTrigger).getByLabelText("Cost telemetry unavailable"),
    ).toHaveTextContent("—");
    await user.click(modelTrigger);
    expect(modelTrigger).toHaveAttribute("aria-expanded", "true");
    const modelPeek = await screen.findByRole("region", {
      name: "gpt-5.6 usage peek",
    });
    ...
    expect(
      within(modelPeek).getByText("Cache read tokens"),
    ).toBeInTheDocument();
    expect(
      within(modelPeek).getByText("Cache write tokens"),
    ).toBeInTheDocument();
    expect(within(modelPeek).getByText("Reasoning tokens")).toBeInTheDocument();
    expect(
      within(modelPeek).getByText("Uncached input tokens"),
    ).toBeInTheDocument();
    expect(within(usageSection).getAllByText("Coming soon")).toHaveLength(1);
    ...
    expect(sectionTitles.indexOf("Usage & cost")).toBe(
      sectionTitles.indexOf("Activity") + 1,
    );
  ...
  it("does not expose backend integrity state as a usage warning", async () => {
    ...
    expect(
      within(usageSection).queryByText("Usage may be incomplete"),
    ).not.toBeInTheDocument();
    expect(
      within(usageSection).queryByLabelText(/Usage may be incomplete/),
    ).not.toBeInTheDocument();
  });
  ...
  it("opens and closes provider and model details with the keyboard", async () => {
    ...
    await user.keyboard("{Enter}");
    expect(
      await screen.findByRole("region", { name: "gpt-5.6 usage peek" }),
    ).toBeInTheDocument();
    expect(modelTrigger).toHaveFocus();

    await user.keyboard(" ");
    expect(
      screen.queryByRole("region", { name: "gpt-5.6 usage peek" }),
    ).not.toBeInTheDocument();
    expect(modelTrigger).toHaveFocus();
  });
  ...
  it("shows multiple agents as compact rows with independent disclosures", async () => {
    ...
    getMock.mockImplementation(async (path: string) => {
      if (path === "/api/v1/usage/sessions/{sessionId}") {
        return {
          data: usageTelemetry({ harnesses: [codex, claude] }),
          error: undefined,
        };
      }
      return { data: { reviewerHandleId: "", reviews: [] }, error: undefined };
    ...
    await waitFor(() =>
      expect(
        within(usageSection).getByLabelText(/Codex usage details/),
      ).toBeInTheDocument(),
    );
    const codexRow = within(usageSection).getByLabelText(/Codex usage details/);
    const claudeRow =
      within(usageSection).getByLabelText(/Claude usage details/);
    expect(within(usageSection).getByText("Codex")).toBeInTheDocument();
    expect(within(usageSection).getByText("Claude")).toBeInTheDocument();
    expect(within(usageSection).queryByText("OpenAI")).not.toBeInTheDocument();
    expect(
      within(usageSection).queryByText("Anthropic"),
    ).not.toBeInTheDocument();
    expect(
      within(usageSection).queryByText("Unknown provider"),
    ).not.toBeInTheDocument();
    expect(within(usageSection).queryByText("gpt-5.6")).not.toBeInTheDocument();
    expect(
      within(usageSection).queryByText("claude-opus-4.1"),
    ).not.toBeInTheDocument();

    await user.click(codexRow);
    const codexPeek = await screen.findByRole("region", {
      name: "Codex usage peek",
    });
    expect(within(codexPeek).getByText("gpt-5.6")).toBeInTheDocument();

    await user.click(claudeRow);
    const claudePeek = await screen.findByRole("region", {
      name: "Claude usage peek",
    });
    expect(within(claudePeek).getByText("claude-opus-4.1")).toBeInTheDocument();
    expect(
      within(claudePeek).getByLabelText(
        "Reasoning tokens telemetry unavailable",
      ),
    ).toHaveTextContent("—");
...
describe("SessionInspector summary reviews", () => {
  ...
  it("links a run to its GitHub review, falling back to the PR when it has none", async () => {
    ...
    await openReviewsSection();
    expect(
      await screen.findByRole("link", { name: /View review/ }),
    ).toHaveAttribute(
      "href",
  ...
  it("shows PRs with unresolved comments but no decisive review", async () => {
    ...
    const autoInjectSwitch = screen.getByRole("switch", {
      name: "Auto inject review comments",
    });
    const reviewsSection = screen.getByText("Reviews").closest("section");
    expect(reviewsSection?.compareDocumentPosition(autoInjectSwitch)).toBe(
  ...
  it("cancels the running review instead of allowing rerun", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /cancel review/i }),
    );
