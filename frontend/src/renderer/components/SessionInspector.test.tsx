...
describe("SessionInspector summary reviews", () => {
	...
	it("persists the auto-inject review feedback preference", async () => {
		renderWithQuery(
			<SessionInspector session={session([pr(3, "open")], { autoInjectReviewFeedback: true })} />,
		);
	...
	it("shows PRs with unresolved comments but no decisive review", async () => {
		...
		const autoInjectSwitch = screen.getByRole("switch", { name: "Auto inject review comments" });
