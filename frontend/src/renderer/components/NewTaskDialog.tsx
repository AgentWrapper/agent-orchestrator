...
export function NewTaskDialog({ open, projectId, onCreated, onOpenChange }: NewTaskDialogProps) {
	...
	const createTask = async (mode?: "tui") => {
		...
		try {
			...
			if (apiError) {
				...
				setCanCreateAsTUI(mode !== "tui" && Boolean(code && CHAT_PREFLIGHT_CODES.has(code)));
				throw new Error(apiErrorMessage(apiError, t("newTask.unableToStart")));
			}
