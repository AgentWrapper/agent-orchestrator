...
export function DaemonFailureBanner({ status }: { status: DaemonStatus }) {
	...
	return <DaemonFailureContent status={status} onDismiss={() => setDismissed(true)} />;
}

function DaemonFailureContent({ status, onDismiss }: { status: DaemonStatus; onDismiss: () => void }) {
	...
	const handleDialogKeyDown = (event: KeyboardEvent<HTMLElement>) => {
		...
		setRetryError(null);
		try {
			await aoBridge.daemon.start();
		} catch (error) {
			setRetryError(error instanceof Error ? error.message : "Failed to retry daemon startup.");
		} finally {
			setRetrying(false);
		}
	};
			...
			>
				...
				<div className="flex items-start gap-3">
					...
					<div className="min-w-0 flex-1">
						<p className="font-medium text-(--color-text-import-title)" id={titleId}>{title}</p>
						<p className="mt-0.5 wrap-break-word text-[var(--color-text-import-muted)]" id={descriptionId}>{daemonFailureMessage(status)}</p>
				...
				{retryError ? <p className="mt-2 text-xs text-error">{retryError}</p> : null}
				{details && detailsOpen ? (
