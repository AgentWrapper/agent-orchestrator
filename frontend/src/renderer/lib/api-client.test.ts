...
describe("normalizeApiOperation", () => {
  it("replaces identifier segments after resource collections", () => {
    expect(normalizeApiOperation("get", "/api/v1/projects/my project id")).toBe(
      "GET /api/v1/projects/:id",
    );
    expect(normalizeApiOperation("POST", "/api/v1/sessions/ao-42/kill")).toBe(
      "POST /api/v1/sessions/:id/kill",
    );
    expect(normalizeApiOperation("PUT", "/api/v1/projects/p1/config")).toBe(
      "PUT /api/v1/projects/:id/config",
    );
    expect(
      normalizeApiOperation("GET", "/api/v1/agents/claude-code/models"),
    ).toBe("GET /api/v1/agents/:id/models");
    expect(
      normalizeApiOperation("POST", "/api/v1/agents/codex/models/refresh"),
    ).toBe("POST /api/v1/agents/:id/models/refresh");
  });

  it("leaves collection and non-resource paths untouched", () => {
    expect(normalizeApiOperation("GET", "/api/v1/projects")).toBe(
      "GET /api/v1/projects",
    );
    expect(normalizeApiOperation("POST", "/api/v1/orchestrators")).toBe(
  ...
  it("keeps static child routes instead of treating them as ids", () => {
    ...
    expect(
      normalizeApiOperation("POST", "/api/v1/notifications/read-all"),
    ).toBe("POST /api/v1/notifications/read-all");
    expect(
      normalizeApiOperation("POST", "/api/v1/orchestrators/delegate"),
    ).toBe("POST /api/v1/orchestrators/delegate");
    expect(normalizeApiOperation("POST", "/api/v1/sessions/cleanup")).toBe(
  ...
  it("keeps workspace file routes aligned with the generated API schema", () => {
    expect(
      normalizeApiOperation("GET", "/api/v1/sessions/ao-42/workspace/files"),
    ).toBe("GET /api/v1/sessions/:id/workspace/files");
    expect(
      normalizeApiOperation("GET", "/api/v1/sessions/ao-42/workspace/file"),
    ).toBe("GET /api/v1/sessions/:id/workspace/file");
    expect(
      normalizeApiOperation("POST", "/api/v1/sessions/ao-42/preview/server"),
    ).toBe("POST /api/v1/sessions/:id/preview/server");
  });

  it("normalizes ids for resources a collection heuristic would miss", () => {
    expect(normalizeApiOperation("GET", "/api/v1/orchestrators/orch-abc")).toBe(
      "GET /api/v1/orchestrators/:id",
    );
    expect(normalizeApiOperation("POST", "/api/v1/prs/pr-1/merge")).toBe(
