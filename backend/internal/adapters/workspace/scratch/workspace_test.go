...
func TestWorkspaceCreatesBranchlessPerSessionDirectories(t *testing.T) {
	...
	if want := filepath.Join(wantRoot, "scratch", "workers", "scratch-1"); worker.Path != want {
	...
	if want := filepath.Join(wantRoot, "scratch", "orchestrators", "scratch-2"); orchestrator.Path != want {
