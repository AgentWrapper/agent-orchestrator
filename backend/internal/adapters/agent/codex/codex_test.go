...
func sessionHookFlags() []string {
	return []string{
		...
		"-c", `hooks.PreToolUse=[{matcher="^request_user_input$",hooks=[{type="command",command="ao hooks codex pre-tool-use",timeout=5}]}]`,
		"-c", `hooks.PostToolUse=[{matcher="^request_user_input$",hooks=[{type="command",command="ao hooks codex post-tool-use",timeout=5}]}]`,
