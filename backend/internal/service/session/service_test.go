...
func TestToAPIErrorMapsWorkspaceBranchSentinels(t *testing.T) {
	cases := []struct {
		...
		{"model unreachable", fmt.Errorf("spawn: %w: harness %q model %q from explicit spawn model: 400 model not available", sessionmanager.ErrModelUnreachable, "codex", "bogus"), apierr.KindInvalid, "MODEL_UNREACHABLE"},
