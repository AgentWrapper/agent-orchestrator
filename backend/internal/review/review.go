...
// TriggerResult is the outcome of a trigger: the (new or existing) run, the
// stable reviewer execution handle, and whether a new pass was started (false
// when an existing run for the same commit was reused).
...
// SessionReviews is a worker's review state: the reviewer handle plus its
...
// CancelResult is the review state after a reviewer cancellation.
...
// heads, and uses one reviewer execution for every new run in the batch.
