...
func TestLauncherSpawnReturnsStableHandle(t *testing.T) {
	...
	if reviewer.gotInv.RunID != "run-1" || reviewer.gotInv.TargetSHA != "sha1" || reviewer.gotInv.ReviewerID != "review-mer-1-run-1" {
...
func TestLauncherSpawnRunsReviewerPreLaunch(t *testing.T) {
	...
	if reviewer.gotPre.ReviewerID != "review-mer-1-run-1" || reviewer.gotPre.WorkspacePath != "/ws/mer-1" {
