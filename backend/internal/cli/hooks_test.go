...
func TestHooks_PiSessionStartReportsAgentSessionIDWithoutActivity(t *testing.T) {
	t.Setenv("AO_SESSION_ID", "ao-7")
	cfg := setConfigEnv(t)
	srv, capture := activityServer(t, http.StatusOK, `{"ok":true}`)
	writeRunFileFor(t, cfg, srv)
	...
	_, _, err := executeCLI(t, Deps{
		In:           strings.NewReader(`{"session_id":"pi-native-1"}`),
		ProcessAlive: func(int) bool { return true },
	...
	want := setActivityAPIRequest{Event: "session-start", AgentSessionID: "pi-native-1"}
	if req != want {
		t.Fatalf("body = %+v, want %+v", req, want)
	}
}
