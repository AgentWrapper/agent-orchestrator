...
func newSpawnCommand(ctx *commandContext) *cobra.Command {
	...
	f.StringVar(&opts.mode, "mode", "", "Session interface: chat (structured agent connection) or tui (the agent's native terminal). Omitted uses the daemon default. Fixed for the session's lifetime.")
