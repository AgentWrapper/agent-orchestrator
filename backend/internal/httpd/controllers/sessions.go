...
func (c *SessionsController) spawn(w http.ResponseWriter, r *http.Request) {
	...
	// displayName is optional at the API, and the read model falls back to the
	// session id. `ao spawn` leaves it empty unless --name is explicit. When
	// present, it is held to the same length cap here so a direct API call cannot
	// exceed it.
	...
	sess, err := c.Svc.Spawn(r.Context(), ports.SpawnConfig{ProjectID: in.ProjectID, IssueID: in.IssueID, Kind: in.Kind, Harness: in.Harness, Branch: in.Branch, Prompt: in.Prompt, Model: in.Model, DisplayName: displayName})
	if err != nil {
		envelope.WriteError(w, r, err)
		return
	}
