...
type SessionService interface {
	...
	SetAutoInjectReviewFeedback(ctx context.Context, id domain.SessionID, autoInject bool) (domain.Session, error)
...
func (c *SessionsController) Register(r chi.Router) {
	...
	r.Patch("/sessions/{sessionId}/auto-inject-review-feedback", c.setAutoInjectReviewFeedbackPolicy)
...
func (c *SessionsController) setAutoInjectReviewFeedbackPolicy(w http.ResponseWriter, r *http.Request) {
	if c.Svc == nil {
		apispec.NotImplemented(w, r, "PATCH", "/api/v1/sessions/{sessionId}/auto-inject-review-feedback")
		return
	}
	var in AutoInjectReviewFeedbackRequest
	if err := decodeJSON(r, &in); err != nil {
		envelope.WriteAPIError(w, r, http.StatusBadRequest, "bad_request", "INVALID_JSON", "Invalid JSON body", nil)
		return
	}
	sess, err := c.Svc.SetAutoInjectReviewFeedback(r.Context(), sessionID(r), in.AutoInjectReviewFeedback)
	if err != nil {
		envelope.WriteError(w, r, err)
		return
	}
	envelope.WriteJSON(w, http.StatusOK, AutoInjectReviewFeedbackResponse{
		OK:                       true,
		SessionID:                sess.ID,
		AutoInjectReviewFeedback: sess.AutoInjectReviewFeedback,
	})
}
