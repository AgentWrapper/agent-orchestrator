...
func TestSessionsAPI_ListSpawnGetAndActions(t *testing.T) {
	...
	body, status, _ = doRequest(t, srv, "POST", "/api/v1/sessions", `{"projectId":"ao","issueId":"ISS-1","kind":"worker","harness":"codex","prompt":"fix","displayName":"my worker","agentConfig":{"model":"gpt-5.5","reasoningEffort":"high","permissions":"auto"}}`)
	...
	if got := svc.lastSpawn.AgentConfig; got.Model != "gpt-5.5" || got.ReasoningEffort != "high" || got.Permissions != domain.PermissionModeAuto {
		t.Fatalf("spawn agent config = %#v, want per-task overrides", got)
	}
