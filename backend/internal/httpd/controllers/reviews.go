...
type AddressedReviewInput struct {
	...
	ThreadIDs []string `json:"threadIds" description:"Exact provider review thread ids AO should reply to and resolve."`
	Body      string   `json:"body" description:"Reply AO should post before resolving the requested review threads."`
}
...
func (c *ReviewsController) addressed(w http.ResponseWriter, r *http.Request) {
	...
	res, err := c.Svc.Addressed(r.Context(), sessionID(r), reviewsvc.AddressedFeedback{RunID: in.RunID, ThreadIDs: in.ThreadIDs, Body: in.Body})
	if err != nil {
		writeReviewError(w, r, err)
		return
	}
