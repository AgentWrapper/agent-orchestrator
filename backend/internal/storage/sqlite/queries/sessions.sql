...
INSERT INTO sessions (
    ...
    preview_url, preview_revision, terminate_on_pr_merge, auto_inject_review_feedback, cleanup_generation,
...
UPDATE sessions SET
    ...
    branch = ?, workspace_path = ?, workspace_repo_path = ?, diff_base_sha = ?, diff_base_ref = ?, runtime_handle_id = ?,
    runtime_launch_id = ?, agent_session_id = ?, prompt = ?,
    preview_url = ?, preview_revision = ?, terminate_on_pr_merge = ?, auto_inject_review_feedback = ?,
    cleanup_generation = ?,
...
SELECT id, project_id, num, issue_id, kind, harness,
    ...
    reviewer_harness, is_pinned, pinned_at, auto_inject_review_feedback,
    session_mode, provider_conversation_id, controller_generation
...
-- name: SetSessionAutoInjectReviewFeedback :execrows
UPDATE sessions SET auto_inject_review_feedback = ?, updated_at = ? WHERE id = ?;
