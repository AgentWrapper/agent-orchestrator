...
const setSessionReviewAutoInjectOff = `-- name: SetSessionReviewAutoInjectOff :execrows
UPDATE sessions SET review_auto_inject_off = ?, updated_at = ? WHERE id = ?
`

type SetSessionReviewAutoInjectOffParams struct {
	ReviewAutoInjectOff bool
	UpdatedAt           time.Time
	ID                  domain.SessionID
}

func (q *Queries) SetSessionReviewAutoInjectOff(ctx context.Context, arg SetSessionReviewAutoInjectOffParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, setSessionReviewAutoInjectOff, arg.ReviewAutoInjectOff, arg.UpdatedAt, arg.ID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
