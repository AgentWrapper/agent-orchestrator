...
const setSessionAutoInjectReviewFeedback = `-- name: SetSessionAutoInjectReviewFeedback :execrows
UPDATE sessions SET auto_inject_review_feedback = ?, updated_at = ? WHERE id = ?
`

type SetSessionAutoInjectReviewFeedbackParams struct {
	AutoInjectReviewFeedback bool
	UpdatedAt                time.Time
	ID                       domain.SessionID
}

func (q *Queries) SetSessionAutoInjectReviewFeedback(ctx context.Context, arg SetSessionAutoInjectReviewFeedbackParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, setSessionAutoInjectReviewFeedback, arg.AutoInjectReviewFeedback, arg.UpdatedAt, arg.ID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
