...
-- status. Sessions default to staying alive after a PR merge unless the user
-- opts into merge-driven termination.
-- +goose StatementBegin
ALTER TABLE sessions ADD COLUMN terminate_on_pr_merge BOOLEAN NOT NULL DEFAULT FALSE;
-- +goose StatementEnd

-- Recreate the sessions update CDC trigger so policy changes fan out a normal
-- session_updated invalidation to the renderer. Consumers refetch the session
-- read model, so the payload is informational.
-- +goose StatementBegin
