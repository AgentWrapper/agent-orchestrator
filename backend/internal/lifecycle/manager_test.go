...
func TestApplyReviewResultDedupsSCMRoundTripByGitHubReviewID(t *testing.T) {
	st := newFakeStore()
	st.sessions["mer-1"] = working("mer-1")
	msg := &fakeMessenger{}
	m := New(st, msg)
	...
	if len(msg.msgs) != 1 {
		t.Fatalf("direct review should send once, got %d", len(msg.msgs))
	}

	// Simulate a daemon restart: the SCM lane must dedup from the persisted PR
	// reaction payload, not only from the first Manager's in-memory state.
	roundTripMsg := &fakeMessenger{}
	roundTrip := New(st, roundTripMsg)
	obs := ports.SCMObservation{
		...
		Review: ports.SCMReviewObservation{
			...
			Reviews: []ports.SCMReviewSummaryObservation{{
				ID: "98765", Author: "ao-reviewer", State: string(domain.ReviewChangesRequest), Body: "fix auth",
			}},
	...
	if err := roundTrip.ApplySCMObservation(ctx, "mer-1", obs); err != nil {
		t.Fatalf("ApplySCMObservation: %v", err)
	}
	if len(roundTripMsg.msgs) != 0 {
		t.Fatalf("SCM round trip should not double-deliver AO review, got %v", roundTripMsg.msgs)
	}
}

func TestApplyReviewResultSCMRoundTripKeepsOtherReviewerComments(t *testing.T) {
	st := newFakeStore()
	st.sessions["mer-1"] = working("mer-1")
	msg := &fakeMessenger{}
	m := New(st, msg)
	...
	obs := ports.SCMObservation{
		...
		Review: ports.SCMReviewObservation{
			...
			Reviews: []ports.SCMReviewSummaryObservation{
				{ID: "98765", Author: "ao-reviewer", State: string(domain.ReviewChangesRequest), Body: "fix auth"},
				{ID: "555", Author: "human-reviewer", State: string(domain.ReviewChangesRequest), Body: "also fix tests"},
			},
			Threads: []ports.SCMReviewThreadObservation{{
				ID: "thread-1", Path: "auth.go", Line: 12,
				Comments: []ports.SCMReviewCommentObservation{
	...
	if len(scmMsg.msgs) != 1 {
		t.Fatalf("human review feedback should still nudge once, got %v", scmMsg.msgs)
	}
	if strings.Contains(scmMsg.msgs[0], "fix auth") {
		t.Fatalf("SCM nudge should filter already-delivered AO comment:\n%s", scmMsg.msgs[0])
	}
