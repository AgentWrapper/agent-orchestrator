...
func TestWiring_StartSessionBuildsSessionService(t *testing.T) {
	...
	svc, reviewSvc, lc, err := startSession(cfg, rt, store, lcm, nil, messenger, telemetryadapter.NoopSink{}, agents, log)
	if err != nil {
...
func TestWiring_StartSessionSpawnsScratchWithoutGitRepo(t *testing.T) {
	...
	svc, _, _, err := startSession(cfg, runtime, store, lcm, nil, messenger, telemetryadapter.NoopSink{}, agents, log)
	if err != nil {
		t.Fatalf("startSession: %v", err)
	}
...
func TestStartSession_SpawnDoesNotPanicWhenNoTrackerToken(t *testing.T) {
	...
	svc, _, _, err := startSession(cfg, rt, store, lcm, nil, messenger, telemetryadapter.NoopSink{}, agents, log)
	if err != nil {
		t.Fatalf("startSession: %v", err)
	}
