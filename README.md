...
On macOS, if the app will not open and says the developer cannot be verified, right-click it and choose **Open**, then **Open** again. Double-clicking only offers Move to Trash. Every macOS build is signed with our Apple Developer ID and notarized; that dialog usually means Gatekeeper could not reach Apple to confirm the notarization, which a VPN, company firewall, or DNS filter can block. `xattr -dr com.apple.quarantine "/Applications/Agent Orchestrator.app"` also clears it.
